	...
	public TestJavaMain()
	{
		...
		if(m_nDLLHandle > 0)
		{
			...
			*/
			...
			// Return_Value_Info_t nGpsXMLLen;
			/*
			Return_Value_Info_t nGpsXMLLen = new Return_Value_Info_t();
			nRet = IDpsdkCore.DPSDK_AskForLastGpsStatusXMLStrCount(m_nDLLHandle, nGpsXMLLen, 10000);
			if(nRet == dpsdk_retval_e.DPSDK_RET_SUCCESS)
			{
				System.out.printf("获取最新GPS信息XML串长度成功，nRet = %d", nRet);
			}else
			{
				System.out.printf("获取最新GPS信息XML串长度失败，nRet = %d", nRet);
			}
			System.out.println();
			*/
			...
			*/
			...
			getInfo.szCameraId = "1000000$1$0$0".getBytes();
			...
			Date tmStart = new Date(2018-1900,11-1,31,0,0,0);
			@SuppressWarnings("deprecation")
			Date tmEnd = new Date(2018-1900,11-1,31,0,1,0);
			//long begintime = tmStart.getTime()/1000;
			//long endtime = tmEnd.getTime()/1000;
			long begintime = 1542960900;
			long endtime = 1542960910;
			...
			nRet = IDpsdkCore.DPSDK_SaveRecordStreamByTime( m_nDLLHandle, nDownLoadSeq, getInfo, scType, "./test.mp4".getBytes(), 60000);
	        ...
	        if (nRet == dpsdk_retval_e.DPSDK_RET_SUCCESS) {
				...
				if(nCount > 0)
				{
					...
					if(records.nCount > 0 && nRet == dpsdk_retval_e.DPSDK_RET_SUCCESS)
					{
						...
						if(nRet == dpsdk_retval_e.DPSDK_RET_SUCCESS)
						{
							System.out.printf("Close file DhApi.RecordStream success! nRet = %d, nSeq = %d", nRet, m_nPlaybackSeq.nReturnValue);
						}else
						{
							System.out.printf("Close file DhApi.RecordStream failed! nRet = %d", nRet);
						}
						System.out.println();
					}
					...
					if(records.nCount > 0 && nRet == dpsdk_retval_e.DPSDK_RET_SUCCESS)
					{
						...
						if(nRet == dpsdk_retval_e.DPSDK_RET_SUCCESS)
						{
							System.out.printf("Close Time DhApi.RecordStream success! nRet = %d, nSeq = %d", nRet, m_nPlaybackSeq.nReturnValue);
						}else
						{
							System.out.printf("Close Time DhApi.RecordStream failed! nRet = %d", nRet);
						}
						System.out.println();
					}
				}
